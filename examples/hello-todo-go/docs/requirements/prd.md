# product requirements document — hello-todo-go

## overview

`hello-todo-go` is a single-user, in-memory todo-list HTTP API built with Go and the Gin
web framework using a hexagonal (ports-and-adapters) architecture. It serves as the
...
- keep the implementation simple enough that a developer can read the entire codebase in
  under one hour
- serve as a stable reference for the hexagonal architecture pattern in Go
...
what a complete set of phase artifacts looks like for a small Go HTTP service.
...
Measured with `go test` benchmarks; no external load-testing required for v0.1.0.
...
Structured logs via `log/slog` (Go stdlib) to stdout. Log level configurable via
`LOG_LEVEL` env var (default: `info`).

## success criteria

- `go test ./...` passes with no failures from a clean checkout
...
- GitHub Actions CI running `go test ./...` and `go vet`
