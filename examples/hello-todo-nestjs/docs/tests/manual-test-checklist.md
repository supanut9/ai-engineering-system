# manual test checklist — hello-todo-nestjs v0.1.0

Run these steps against a locally running server before every release. Start the server
with `make run` (or `make build && node dist/main.js`). Default address:
`http://localhost:3000`.
...
- [ ] `make build` produces `dist/main.js`.
- [ ] Server starts: `node dist/main.js` prints a log line mentioning the port.
...
```bash
curl -s -X POST http://localhost:3000/v1/todos \
...
```bash
curl -s http://localhost:3000/v1/todos
```
...
```bash
curl -s http://localhost:3000/v1/todos/<id>
```
...
```bash
curl -s http://localhost:3000/v1/todos/doesnotexist
```
...
```bash
curl -s -X PATCH http://localhost:3000/v1/todos/<id> \
  -H 'Content-Type: application/json' \
...
```bash
curl -s -X PATCH http://localhost:3000/v1/todos/<id-with-due-at> \
  -H 'Content-Type: application/json' \
  -d '{"due_at":null}'
```
...
```bash
curl -s -X PATCH http://localhost:3000/v1/todos/ghost \
  -H 'Content-Type: application/json' \
  -d '{"title":"x"}'
```
...
```bash
curl -s -X DELETE http://localhost:3000/v1/todos/<id>
```
...
```bash
curl -s -X DELETE http://localhost:3000/v1/todos/ghost
```
