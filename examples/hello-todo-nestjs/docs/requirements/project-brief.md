# project brief — hello-todo-nestjs
...
> like for a small NestJS HTTP service.
...
Developers learning the ai-engineering-system who want a NestJS reference they can
compare against their own phase artifacts and against the `hello-todo-go` reference.

## goal

Deliver a minimal, single-user, in-memory todo-list HTTP API in TypeScript (NestJS +
layered architecture) that walks Phase 0 through Phase 8 of the ai-engineering-system
workflow. Every artifact in the example should be coherent, realistic, and useful as a
model.
...
|---|---|---|
| example bit-rots as NestJS versions update | medium | pin versions in package.json; dependabot on CI |
| example grows too complex and loses clarity | low | enforce one-day scope cap; reject scope additions |
| layered structure confuses developers expecting hexagonal | low | ADR-0001 explains the choice and what it teaches |
