# go-live checklist — hello-todo-nestjs v0.1.0

Work through this list top to bottom before directing any traffic to the new process.

## engineering readiness

- [ ] `make test` passes on the release commit — all suites `PASS`.
- [ ] CI pipeline is green for the release commit or tag.
- [ ] `make lint` exits zero (no TypeScript errors).
- [ ] `make build` produces `dist/main.js` without error.
- [ ] `node dist/main.js` starts and responds: `curl http://localhost:3000/healthz`
      returns `{"status":"ok"}`.
