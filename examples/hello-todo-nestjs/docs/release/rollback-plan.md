# rollback plan — hello-todo-nestjs v0.1.0
...
the `dist/` directory and restarting the process.

1. Stop the running process:
   ```bash
   kill -TERM <pid>
   # or: systemctl stop hello-todo-nestjs
   ```
   Wait for the process to exit (graceful NestJS shutdown).

2. Restore the previous dist directory:
   ```bash
   cp -r /opt/hello-todo-nestjs/dist.previous /opt/hello-todo-nestjs/dist
   ```
...
Full rollback (replace dist and restart): appropriate for any trigger condition listed
above.
