...
- hello-todo-nestjs (the whole example)
...
- Makefile targets: `setup`, `run`, `test`, `test-e2e`, `lint`, `fmt`, `build`
- CI uses `actions/setup-node@v5` with Node 22 and caches npm
- `make lint` runs `npx tsc --noEmit`; Phase 3 tooling will add eslint
