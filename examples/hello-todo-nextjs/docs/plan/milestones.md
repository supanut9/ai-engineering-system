...
|---|---|
| milestone | v0.1.0 |
| description | first runnable todo app with all six API endpoints, server-rendered home page, in-memory storage, and passing tests |
...
- US-001: create a todo with a title → POST /api/todos returns 201 with correct body
- US-002: create a todo with an optional due date → `due_at` is persisted and returned
- US-003: list all todos → GET /api/todos returns 200 with `{"items":[...]}`
...
- US-009: home page renders todos server-side → HTML contains todo titles

### exit criteria

- `npm test` passes with no failures
- `make dev` starts the server and `GET /healthz` returns `{"status":"ok"}`
- `GET /` renders an HTML page listing todos
...
- persistent storage backend (SQLite or Postgres adapter)
- `GET /api/todos?completed=true` filtering
- pagination on list endpoint
- Docker image and container deployment
- client-side mutations via React Server Actions
