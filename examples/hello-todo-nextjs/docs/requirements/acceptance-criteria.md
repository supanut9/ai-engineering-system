# acceptance criteria — hello-todo-nextjs
...
**Given** a POST request to `/api/todos` with body `{"title":"Buy milk"}`
...
- response body contains a todo object with:
  ...
  - `updated_at`: valid ISO8601 timestamp equal to `created_at`
- a subsequent `GET /api/todos/{id}` with the returned id returns the same object

---

## US-002: create a todo with an optional due date

**Given** a POST request to `/api/todos` with body
...
**When** `PATCH /api/todos/abc` is called with body `{"completed":true}`
**Then**
- status is 200
...
**Given** a todo with id `abc` exists
**When** `DELETE /api/todos/abc` is called
**Then**
- status is 204
- response body is empty
- a subsequent `GET /api/todos/abc` returns 404

**Given** no todo with id `xyz` exists
**When** `DELETE /api/todos/xyz` is called
**Then**
- status is 404
- body is `{"error":{"code":"not_found","message":"todo not found"}}`
...
**Then**
- status is 400
- body is `{"error":{"code":"validation_error","message":"..."}}`
...
- `GET /api/todos/{unknown}` → 404 with `not_found` error body
- `PATCH /api/todos/{unknown}` → 404 with `not_found` error body
- `DELETE /api/todos/{unknown}` → 404 with `not_found` error body
...
## US-009: view todos on the home page

**Given** two todos exist with titles `"Buy milk"` and `"Call dentist"`
**When** `GET /` is requested
**Then**
- HTTP status is 200
- the response is HTML
- the HTML contains the text `"Buy milk"` and `"Call dentist"`
- no server error occurs during rendering

**Given** zero todos exist
**When** `GET /` is requested
**Then**
- HTTP status is 200
- the HTML contains an empty-state indicator (e.g. "No todos yet")
