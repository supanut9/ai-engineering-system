# functional specification — hello-todo-rust
...
Ids are opaque strings generated server-side using `uuid::Uuid::new_v4().simple()`
(32 lowercase hex characters, no hyphens). Callers must not infer ordering or
structure from the id value.
...
- Extra JSON fields not listed above are silently ignored.

### success behavior

- server generates a unique opaque id via `uuid::Uuid::new_v4().simple()`
...
- order of items follows insertion order (maintained by the store's auxiliary id vector)
...
| completely unknown route | 404 (Axum default) |
