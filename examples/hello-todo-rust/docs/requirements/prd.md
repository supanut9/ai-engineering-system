# product requirements document — hello-todo-rust

## overview

`hello-todo-rust` is a single-user, in-memory todo-list HTTP API built with Rust and the
Axum web framework using a hexagonal (ports-and-adapters) architecture. It serves as the
canonical filled-in example for the ai-engineering-system, demonstrating what every
workflow phase looks like for a real — if intentionally tiny — project.
...
- keep the implementation simple enough that a developer can read the entire codebase in
  under one hour
- serve as a stable reference for the hexagonal architecture pattern in Rust
...
what a complete set of phase artifacts looks like for a small Rust HTTP service.
...
Measured with `cargo bench` or load tests; no external load-testing required for v0.1.0.
...
Structured logs via `tracing` + `tracing-subscriber` to stdout. Log level configurable
via `RUST_LOG` env var (default: `info`).

## success criteria

- `cargo test` passes with no failures from a clean checkout
...
- GitHub Actions CI running `cargo test`, `cargo clippy`, and `cargo fmt --check`
...
1. Should a later version support sub-tasks (nesting a todo under another todo)? The
   ...
   trait and repository contract would need to change. Deferred to v0.2.0 discussion.
