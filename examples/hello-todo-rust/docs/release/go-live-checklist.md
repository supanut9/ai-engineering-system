# Go-Live Checklist — hello-todo-rust v0.1.0

Work through this list top to bottom before directing any traffic to the new binary.

## Engineering readiness

- [ ] `make test` passes on the release commit — all test functions pass.
- [ ] CI pipeline is green for the release commit or tag.
- [ ] `make clippy` produces no output (zero warnings treated as errors).
- [ ] `make fmt` produces no diff output.
- [ ] `make build` produces `target/release/my-service` without error.
- [ ] Binary starts and responds: `curl http://localhost:8080/healthz` returns `{"status":"ok"}`.

## Configuration

- [ ] `PORT` environment variable is set correctly in the target environment (default `8080`).
- [ ] `RUST_LOG` is set to an appropriate level (`info` for production).
