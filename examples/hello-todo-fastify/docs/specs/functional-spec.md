# functional specification — hello-todo-fastify

Behavioral contract for each endpoint. Supplements the acceptance criteria in
`../requirements/acceptance-criteria.md`. Implementation must match this exactly.
...
Fastify enforces the `Content-Type` check automatically when the route schema declares
a body type via `fastify-type-provider-zod`.
...
Ids are opaque strings generated server-side (UUID v4 via the `uuid` package). Callers
must not infer ordering or structure from the id value.
...
- Extra JSON fields not listed above are silently ignored (Zod `.strip()` default).

Validation of `title` trim and length is enforced by the core service, not by Zod, so
that the rule lives in a single place regardless of how many inbound adapters exist.
Zod at the HTTP boundary checks structural types (string, optional string) and delegates
content rules to the service.

### success behavior

- server generates a unique UUID v4 id
...
- order of items is not guaranteed (Map iteration order is insertion order in V8, but
  the contract does not guarantee this)
...
Distinguishing "field absent" from "field present as null" is handled by the Zod schema
using `.optional()` (absent) versus allowing `.nullable()` (explicitly null). The core
service receives a typed patch object and only applies present fields.

### validation rules

- if `title` is supplied, same rules apply as POST: non-empty after trim, ≤ 200 chars
- if `due_at` is supplied as a string, it must be valid RFC3339
...
| missing Content-Type on POST/PATCH | 400 validation_error |
| completely unknown route | 404 (Fastify default) |
