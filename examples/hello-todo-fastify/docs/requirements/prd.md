# product requirements document — hello-todo-fastify

## overview

`hello-todo-fastify` is a single-user, in-memory todo-list HTTP API built with
TypeScript and the Fastify web framework using a hexagonal (ports-and-adapters)
architecture. It serves as the Node.js/TypeScript counterpart to `hello-todo-go`,
demonstrating the same 8-phase ai-engineering-system workflow artifacts filled in for
a different runtime. The API surface is intentionally identical to `hello-todo-go`.
...
- keep the implementation simple enough that a developer can read the entire codebase in
  under one hour
- serve as a stable reference for the hexagonal architecture pattern in TypeScript, using
  Fastify + Zod for typed route validation
...
what a complete set of phase artifacts looks like for a small TypeScript HTTP service.
...
No external load-testing required for v0.1.0; Vitest benchmarks are sufficient.
...
only for v0.1.0. This envelope is identical to `hello-todo-go`.
...
The server binds to `:8080` by default. The bind port is overridable via the `PORT`
environment variable.

### NFR-05: logging

Structured logs via `pino` (Fastify's built-in logger) to stdout. Log level configurable
via `LOG_LEVEL` env var (default: `info`). Log format is JSON in production; optionally
...
- `make run`, `make test`, `make lint` Makefile targets
- GitHub Actions CI running Vitest and `tsc --noEmit`
...
- OpenAPI / Swagger spec generation (Fastify supports this via `@fastify/swagger` but
  it is out of scope for v0.1.0)
