...
- hello-todo-fastify (the whole example)
...
- Makefile targets: `setup`, `run`, `test`, `lint`, `fmt`, `build`
- CI uses `actions/setup-node@v5` with Node 22 and caches `~/.npm`
- `tsc --noEmit` in the lint target catches type errors without emitting files
